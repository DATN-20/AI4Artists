...
import { useBrushSizeState } from "@/store/canvasHooks"
import { RxBorderWidth } from "react-icons/rx"
import * as Slider from "@radix-ui/react-slider"

const BrushSizeInput = () => {
  const { brushSettings, setBrushSettings } = useBrushSizeState()
          ...
          <div className="absolute left-0 top-0 z-10 mt-[-30px] flex h-7 w-[200px] items-center rounded bg-stone-600">
            ...
            >
              <Slider.Track className="relative h-3 grow rounded-xl bg-slate-200">
                <Slider.Range className="absolute h-3 rounded-xl bg-gradient-linear" />
              </Slider.Track>
              <Slider.Thumb className="group relative block h-4 w-4 cursor-pointer rounded-full border-none bg-primary outline-none duration-150 active:scale-125 ">
