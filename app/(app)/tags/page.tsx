...
const TagsDisplay: React.FC<{ tags: string }> = ({ tags }) => {
  ...
  const formattedTags = tagsArray.map(tag => tag.replace(/_/g, ' '));
  return (
    <div className="flex flex-wrap">
      {formattedTags.map((tag, index) => (
        <span key={index} className="bg-gray-300 text-black hover:text-primary-500 text-sm px-2 py-1 m-1 rounded">
          {tag}
        </span>
      ))}
    </div>
